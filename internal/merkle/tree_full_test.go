...
func TestHashPayload(t *testing.T) {
	...
	if err != nil { t.Fatalf("HashPayload: %v", err) }
	if hash == "" { t.Error("expected non-empty hash") }

	// Same input should produce same hash (deterministic)
	hash2, _ := tree.HashPayload(context.Background(), "hello world")
	if hash != hash2 { t.Error("hash should be deterministic") }
}
...
func TestCreateRoot_SingleLeaf(t *testing.T) {
	...
	if err != nil { t.Fatalf("CreateRoot: %v", err) }
	if root == "" { t.Error("expected non-empty root") }
}
...
func TestWitness_InvalidPosition(t *testing.T) {
	...
	if err == nil { t.Error("expected error for position 0") }

	_, err = tree.GenerateRoute(context.Background(), []string{"a", "b"}, 3)
	if err == nil { t.Error("expected error for out-of-range position") }
}
...
func TestCreateRoot_ContractNotFound(t *testing.T) {
	...
	if err == nil { t.Error("expected error for missing contract") }
}
...
func TestNewTree(t *testing.T) {
	...
	if tree == nil { t.Error("expected non-nil tree") }
}
...
func TestCreateRoot_Deterministic(t *testing.T) {
	...
	if r1 != r2 { t.Error("Merkle root should be deterministic") }
}
...
func TestWitness_TamperedProof(t *testing.T) {
	...
	if err != nil { t.Fatalf("GenerateRoute: %v", err) }
	...
	if err != nil { t.Fatalf("Witness: %v", err) }
	if result.Valid { t.Error("expected invalid for tampered proof") }
}
...
func TestCreateTreeWithProof(t *testing.T) {
	...
	if err != nil { t.Fatalf("CreateTreeWithProof: %v", err) }
	if root == "" { t.Error("expected non-empty root") }
}
