...
func zb() string {
	for _, p := range []string{"zenroom", "/usr/bin/zenroom", "/usr/local/bin/zenroom"} {
		if _, err := exec.LookPath(p); err == nil { return p }
	}
	return ""
}
...
func newTS(t *testing.T) *Server {
	t.Helper()
	bin := zb()
	if bin == "" { t.Skip("zenroom not found") }
...
func TestHealth(t *testing.T) {
	...
	if resp["status"] != "ok" { t.Errorf("expected ok, got %q", resp["status"]) }
}
...
func TestOpenAPI(t *testing.T) {
	...
	if w.Code != 200 { t.Errorf("expected 200, got %d", w.Code) }
	if len(w.Body.Bytes()) < 100 { t.Error("expected substantial openapi spec") }
}
...
func TestListContracts(t *testing.T) {
	...
	json.Unmarshal(w.Body.Bytes(), &resp)
	if len(resp.Contracts) == 0 { t.Error("expected at least one contract") }
	if resp.Directory == "" { t.Error("expected directory") }
}
...
func TestGetContract_PathTraversal(t *testing.T) {
	...
	if w.Code != 400 && w.Code != 307 { t.Errorf("expected 400 or 307, got %d", w.Code) }
}
...
func TestRemember_EmptyPayload(t *testing.T) {
	...
	if w.Code != 400 { t.Errorf("expected 400, got %d: %s", w.Code, w.Body.String()) }
}
...
func TestRecall_AfterRemember(t *testing.T) {
	...
	var m1 struct{ MemoryID string `json:"memory_id"` }
	json.Unmarshal(w1.Body.Bytes(), &m1)
	// Recall
	w2 := doReq(s, "GET", "/memories/"+m1.MemoryID, nil)
	if w2.Code != 200 { t.Errorf("expected 200, got %d", w2.Code) }
	var m2 struct{ Payload string `json:"payload"` }
	json.Unmarshal(w2.Body.Bytes(), &m2)
	if m2.Payload != "test recall" { t.Errorf("expected 'test recall', got %q", m2.Payload) }
}
...
func TestAnchorBeacon_WithMemories(t *testing.T) {
	...
	if beacon.ProofCount != 2 { t.Errorf("expected 2, got %d", beacon.ProofCount) }
}
...
func TestAnchorBeacon_UpdatesMemoryBeaconID(t *testing.T) {
	...
	if m.BeaconID != "current" { t.Errorf("new memory should have beacon 'current', got %q", m.BeaconID) }
}
...
func TestGenerateRoute_AfterAnchor(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		var m struct{ MemoryID string `json:"memory_id"` }
	...
	if len(route.Path) == 0 { t.Error("expected non-empty proof path") }
	if route.Position == 0 { t.Error("expected non-zero position") }
	if route.LeafCount != 4 { t.Errorf("expected 4, got %d", route.LeafCount) }
}
...
func TestWitness_ValidProof(t *testing.T) {
	...
	if w3.Code != 200 { t.Errorf("expected 200, got %d: %s", w3.Code, w3.Body.String()) }
	var result struct{ Valid bool `json:"valid"` }
	json.Unmarshal(w3.Body.Bytes(), &result)
	if !result.Valid { t.Logf("proof was not valid, route data: %s", string(verifyBody)) }
}
...
func TestHandler(t *testing.T) {
	s := newTS(t)
	h := s.Handler()
	if h == nil { t.Error("expected non-nil handler") }
}
...
func TestWitness_ZeroPosition(t *testing.T) {
	...
	if w.Code != 200 && w.Code != 400 && w.Code != 500 { t.Errorf("unexpected code: %d", w.Code) }
}
...
func TestRememberAndRecallFlow(t *testing.T) {
	...
	if w1.Code != 201 { t.Fatalf("Remember: %d", w1.Code) }
	var m1 struct{ MemoryID string `json:"memory_id"` }
	json.Unmarshal(w1.Body.Bytes(), &m1)

	// Recall
	w2 := doReq(s, "GET", "/memories/"+m1.MemoryID, nil)
	if w2.Code != 200 { t.Fatalf("Recall: %d", w2.Code) }
}
...
func TestAnchorAndProofFlow(t *testing.T) {
	...
	// Anchor
	w := doReq(s, "POST", "/checkpoints", nil)
	if w.Code != 201 { t.Fatalf("Anchor: %d: %s", w.Code, w.Body.String()) }
	var beacon domain.Beacon
	json.Unmarshal(w.Body.Bytes(), &beacon)
	if beacon.ProofCount != 4 { t.Errorf("expected 4, got %d", beacon.ProofCount) }
}
...
func TestWitness_CompleteFlow(t *testing.T) {
	...
	if w.Code != 200 { t.Skipf("route gen failed: %d", w.Code); return }
	...
	if w2.Code != 200 { t.Errorf("verify failed: %d", w2.Code) }
}
...
func TestRecall_InvalidID(t *testing.T) {
	...
	if w.Code != 404 { t.Errorf("expected 404 for invalid ID, got %d", w.Code) }
}
...
func TestGenerateRoute_SingleMemory(t *testing.T) {
	...
	w2 := doReq(s, "GET", "/proofs/"+m.MemoryID, nil)
	if w2.Code != 200 { t.Skipf("route with single leaf: %d", w2.Code) }
}
...
func TestCORSPreflight_Direct(t *testing.T) {
	...
	if w.Code == 0 { t.Error("expected non-zero status code") }
}
