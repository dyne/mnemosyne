...
func TestRun_InvalidScript(t *testing.T) {
	...
	if err == nil { t.Error("expected error for invalid script") }
}
...
Then print the 'hash'`)
	...
	if err != nil { t.Fatalf("Run with keys: %v", err) }
...
Then print the 'key derivations'`)
	...
	if err != nil { t.Fatalf("OutputMap: %v", err) }
	if _, ok := m["key_derivations"]; !ok { t.Error("expected key_derivations in output") }
}
...
func TestRunLua(t *testing.T) {
	...
	if err != nil { t.Fatalf("RunLua: %v", err) }
	m, err := result.OutputMap()
	if err != nil { t.Fatalf("OutputMap: %v", err) }
	if m["status"] != "ok" { t.Errorf("expected ok, got %v", m["status"]) }
}
...
func TestRunLua_WithData(t *testing.T) {
	...
	m, err := result.OutputMap()
	if err != nil { t.Fatalf("OutputMap: %v", err) }
	if m["from_lua"] != true { t.Error("expected from_lua flag") }
}
...
func TestRunLua_EmptyOutput(t *testing.T) {
	...
	if err == nil { t.Error("expected error for empty output") }
}
...
func TestResult_OutputMap_NotObject(t *testing.T) {
	...
	_, err := r.OutputMap()
	if err == nil { t.Error("expected error for non-object output") }
}

func TestResult_OutputString(t *testing.T) {
	r := &Result{Raw: []byte("hello")}
	if r.OutputString() != "hello" { t.Error("OutputString mismatch") }
}
...
func TestWriteTemp(t *testing.T) {
	...
	if err != nil { t.Fatalf("writeTemp: %v", err) }
	defer os.Remove(path)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		t.Error("temp file does not exist")
	}
	data, err := os.ReadFile(path)
	if err != nil { t.Fatalf("read temp: %v", err) }
	if string(data) != `{"test":true}` { t.Error("content mismatch") }
}
